...
{
    public interface IGlobalDomainRefreshTokenRepository
    {
    ...
    public class GlobalDomainRefreshTokenRepository : IGlobalDomainRefreshTokenRepository
    {
        private readonly string _connectionString;

        public GlobalDomainRefreshTokenRepository(IConfiguration config)
        {
            string environment = config.GetValue<string>("GBLEnvironment")!.ToUpper().Trim();

            _connectionString = config.GetGblConnectionString();
        }
        ...
        public async Task StoreRefreshTokenAsync(RefreshTokenRecord record)
        {
            ...
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_Insert", con)
            {
        ...
        {
            await using var con = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_GetByHash", con)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        {
            await using var con = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_RevokeOne", con)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        {
            await using var con = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_RevokeFamily", con)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        {
            await using var con = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_GetActiveSession", con)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        {
            await using var con = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand("usp_GBL_UserRefreshTokens_GetBySessionId", con)
            { CommandType = CommandType.StoredProcedure };
