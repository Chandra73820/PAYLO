...
{
    public interface IGlobalDomainTokenService
    {
    ...
    public class GlobalDomainTokenService : IGlobalDomainTokenService
    {
        private readonly IConfiguration _config;
        private readonly IGlobalDomainRefreshTokenRepository _Gblrepo;
        private readonly IGlobalDomainAccessTokenBlacklist _blacklist;
        ...
        public GlobalDomainTokenService(IConfiguration config,
                            IGlobalDomainRefreshTokenRepository repo,
                            IGlobalDomainAccessTokenBlacklist blacklist)
        {
            _config = config;
            _Gblrepo = repo;
    ...
    string userType, string ipAddress, string userAgent)
        {
            ...
            await _Gblrepo.StoreRefreshTokenAsync(new RefreshTokenRecord
            {
                RegID = regId,
                IdNo = idNo,
                UserType = userType,
        ...
        public async Task<TokenPair?> RotateRefreshTokenAsync(string rawRefreshToken, string userType, string ipAddress, string userAgent)
        {
            ...
            var record = await _Gblrepo.GetByHashAsync(hash, userType);

            if (record == null || record.IsRevoked || record.ExpiresAt < DateTime.UtcNow)
            {
                if (record?.IsRevoked == true)
                    await _Gblrepo.RevokeUserFamilyAsync(
            ...
            try
            {
                ...
                var recheck = await _Gblrepo.GetByHashAsync(hash, userType);
                if (recheck == null || recheck.IsRevoked)
                    return null;

                await _Gblrepo.RevokeTokenAsync(hash, record.IdNo!, userType, replacedBy: null);
    ...
    string rawRefreshToken, string userType, string? rawJwt)
        {
            ...
            await _Gblrepo.RevokeTokenAsync(
        ...
        {
            string hash = Hash(rawRefreshToken);
            await _Gblrepo.RevokeTokenAsync(hash, idNo, userType, replacedBy: null);
        }
        ...
        public async Task RevokeAllUserTokensAsync(string IdNo, int regId, string userType)
            => await _Gblrepo.RevokeAllAsync(IdNo, regId, userType);
        ...
        {
            var record = await _Gblrepo.GetBySessionIdAsync(multiSessionId, userType);
            if (record == null || record.IsRevoked || record.ExpiresAt < DateTime.UtcNow)
                return null;
            var jti = Guid.NewGuid().ToString();
